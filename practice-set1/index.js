...
const fakeFetch = (url) => {
  ...
  fakeFetch(`https://example.com/api/itemlist`)
  .then(res => res.data)
  .then( (ans) => ans.map(item => {
    output8.innerHTML += `<li>${item.itemName} ---INR ${item.price} --- ${item.quantity}</li>`
  }))

  fakeFetch(`https://example.com/api/itemlist`)
  .then(res => res.data.forEach(item => {
    let list = document.createElement('li')
    list.innerText = `${item.itemName} ---INR ${item.price} --- ${item.quantity}`
    output8.appendChild(list)
  }))
